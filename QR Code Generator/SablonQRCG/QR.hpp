...
namespace qr {
	...
	// verific daca poate fi encodat in alphanumeric
	...
	// verific daca poate fi encodat in numeric
	...
	// verific daca poate fi encodat in kanji
    ...
    // Reed solomon generator pentru Galois Field
     void gf_gen_poly(int degree, uint8_t* poly)
    {
	 ...
	 // diviziune polinomiala in Galois Field
    ...
    // selectare mod encoding pentru string
    ...
    // Returneaza dimensiunea indicatorului de control al caracterelor in biti pentru versiunea si modul dat.
    ...
    private:
        static_assert(V >= 1 && V <= 40, "versiune invalid");
        ...
        static constexpr int N_BYTES = bytes_in_bits(N_BITS);        // Numarul real de bytes_in_bits necesar pentru stocarea intregului cod Qr
        static constexpr int N_DAT_BYTES = bytes_in_bits(N_DAT_BITS);    // Numarul real de bytes_in_bits necesari pentru stocarea [date + ecc]
        static constexpr int N_DAT_CAPACITY = N_DAT_BITS >> 3;              // Capacitate de [date + ecc] fara biti ramasi
        uint8_t code[N_BYTES] = {};
        bool status = false;
    };

    // Obtineti culoarea unui modul de la stanga la dreapta si de sus în jos. Negrul este adevarat.
    template<int V>
    ...
    // Creati un cod Qr cu un nivel de corectare a erorilor dat. Daca masca == -1,
    ...
    void Qr<V>::add_patterns()
    {
        ...
        // Perforati modele de sincronizare
